...
while (($#)); do
  case "$1" in
    ...
    --variant) need_value "$@"; shift; VARIANT="${1^^}" ;;
...
resolve_release() {
  local requested="$1" variant="$2" api tmp
  ...
  curl -fsSL --retry 3 --connect-timeout 20 "$api" -o "$tmp" \
...
if [[ "$MODE" == download ]]; then
  mapfile -t release < <(resolve_release "$VERSION" "$VARIANT")
  (( ${#release[@]} == 2 )) || die "Could not parse OpenCore release metadata"
  resolved_version="${release[0]}"
  url="${release[1]}"
