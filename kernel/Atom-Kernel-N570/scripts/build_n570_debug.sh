...
log() { printf '[atom-kernel-debug-build] %s\n' "$*"; }
die() { printf '[atom-kernel-debug-build] ERROR: %s\n' "$*" >&2; exit 1; }
...
if [ "${CLEAN_BUILD:-0}" = "1" ]; then
  log "CLEAN_BUILD=1: removing previous N570 DEBUG work tree"
  rm -rf "$WORK_DIR"
fi
rm -rf "$ARTIFACT_DIR"
...
log "N570 DEBUG build complete: $ARTIFACT_DIR/mach_kernel"
cat "$ARTIFACT_DIR/mach_kernel.sha256"
log "next: bash '$SCRIPT_DIR/test_n570_debug.sh' '$ARTIFACT_DIR/mach_kernel'"
