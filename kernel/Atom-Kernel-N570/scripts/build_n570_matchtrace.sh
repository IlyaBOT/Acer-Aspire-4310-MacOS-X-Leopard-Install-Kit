...
WORK_DIR="$ROOT_DIR/work/n570-matchtrace"
ARTIFACT_DIR="$ROOT_DIR/artifacts/n570-matchtrace"
BUILD_TOOLS_BIN="$ROOT_DIR/work/build-tools/bin"
SDKROOT="${SDKROOT:-/Developer/SDKs/MacOSX10.6.sdk}"
IOKIT_SOURCE="$SRC_DIR/iokit/Kernel/IOService.cpp"
IOKIT_BACKUP="$ROOT_DIR/work/IOService.cpp.before-matchtrace.$$"

# shellcheck disable=SC1090
. "$LOCK_FILE"

log() { printf '[atom-kernel-matchtrace-build] %s\n' "$*"; }
die() { printf '[atom-kernel-matchtrace-build] ERROR: %s\n' "$*" >&2; exit 1; }
have() { command -v "$1" >/dev/null 2>&1; }

restore_iokit() {
  if [ -f "$IOKIT_BACKUP" ]; then
    cp "$IOKIT_BACKUP" "$IOKIT_SOURCE"
    rm -f "$IOKIT_BACKUP"
    log "restored original local IOService.cpp"
  fi
}
trap restore_iokit EXIT INT TERM
...
have python || die "system Python is required"
...
[ -f "$IOKIT_SOURCE" ] || die "IOService.cpp not found"
...
grep -A2 'case CPUID_MODEL_ATOM:' "$SRC_DIR/osfmk/i386/cpuid.c" | grep -q 'CPUFAMILY_INTEL_YONAH' || die "Atom model 28 must use the historical Yonah compatibility family"
grep -F -q '[N570 ATOM-KERNEL]' "$SRC_DIR/osfmk/i386/i386_init.c" || die "N570 Atom instrumentation markers not found"

mkdir -p "$ROOT_DIR/work"
cp "$IOKIT_SOURCE" "$IOKIT_BACKUP"
python "$SCRIPT_DIR/apply_n570_iokit_matchtrace_patch.py"
grep -F -q '[N570 ATOM-KERNEL][MATCH] F>' "$IOKIT_SOURCE" || die "IOKit matchtrace patch did not apply"
grep -F -q '[N570 ATOM-KERNEL][MATCH] S<' "$IOKIT_SOURCE" || die "IOKit start trace markers are incomplete"
...
if [ "${CLEAN_BUILD:-0}" = "1" ]; then
  log "CLEAN_BUILD=1: removing previous N570 MATCHTRACE work tree"
  rm -rf "$WORK_DIR"
fi
rm -rf "$ARTIFACT_DIR"
...
log "configuration: RELEASE I386 + targeted IOKit match tracing"
log "trace targets: IOResources, bios"
log "SDK: $SDKROOT"
log "MAKEJOBS: $JOBS"
...
git -C "$ROOT_DIR/../.." diff -- \
  kernel/Atom-Kernel-N570/src/xnu/osfmk/i386/cpuid.h \
  kernel/Atom-Kernel-N570/src/xnu/osfmk/i386/cpuid.c \
  kernel/Atom-Kernel-N570/src/xnu/osfmk/i386/i386_init.c \
  kernel/Atom-Kernel-N570/src/xnu/iokit/Kernel/IOService.cpp \
  > "$ARTIFACT_DIR/source.patch" 2>/dev/null || true

shasum -a 256 "$ARTIFACT_DIR/mach_kernel" > "$ARTIFACT_DIR/mach_kernel.sha256"

log "N570 MATCHTRACE RELEASE build complete: $ARTIFACT_DIR/mach_kernel"
cat "$ARTIFACT_DIR/mach_kernel.sha256"
log "IOService.cpp will now be restored by the EXIT trap"
log "next: bash '$SCRIPT_DIR/test_n570_matchtrace.sh' '$ARTIFACT_DIR/mach_kernel'"
