...
KERNEL="${1:-$ROOT_DIR/artifacts/n570-release/mach_kernel}"
SYMBOL_IMAGE="$ROOT_DIR/artifacts/n570-release/mach_kernel.sys"

# shellcheck disable=SC1090
. "$LOCK_FILE"

log() { printf '[atom-kernel-release-test] %s\n' "$*"; }
die() { printf '[atom-kernel-release-test] ERROR: %s\n' "$*" >&2; exit 1; }
...
grep -a -F -q 'RELEASE_I386' "$KERNEL" || die "kernel banner does not identify a RELEASE_I386 build"
if grep -a -F -q 'DEBUG_I386' "$KERNEL"; then
  die "DEBUG_I386 marker found in RELEASE artifact"
fi
...
grep -a -F -q '[N570 ATOM-KERNEL]' "$KERNEL" || die "N570 instrumentation prefix not found in RELEASE kernel"
...
log "PASS instrumentation prefix is embedded in RELEASE kernel"
...
log "N570 RELEASE STATIC VALIDATION: PASS"
log "next: boot this exact artifact with the QEMU Atom profile and compare against the validated vanilla RELEASE control"
