...
> **Current physical-target status (2026-09-16): blocked / not known working.**
> The Linux tooling can build and write the installer media, but the current D640
> test unit has not produced a successful OpenCore/macOS boot. The remaining
> blocker may be failing hardware, unusually hostile Phoenix legacy-BIOS behavior,
> or both. Keep this target as a research/diagnostic profile; do not describe it
> as a working Hackintosh until a real boot is reproduced.
...
The intended path is **Snow Leopard 10.6.3 -> 10.6.8 Combo Update**. Vanilla
...
`--download` also downloads and extracts the historical nawcom/qoopz AMD kernel
packages for 10.6.3 and 10.6.8. The downloader tries the historical original URL
and then queries the Internet Archive for a captured copy if the original host
is gone.
...
The shared Linux backend (`scripts/linux_make_usb.sh`):
...
This confirms the repository has a Linux USB-writing path; it does **not** prove
that the current D640 firmware will execute the resulting media.
...
Update order on a hypothetical running 10.6.3 system:
...
The higher-priority unresolved issue is earlier than those drivers: establish
whether the current laptop is electrically healthy and whether its Phoenix BIOS
can execute any of the tested legacy USB boot layouts reliably.
