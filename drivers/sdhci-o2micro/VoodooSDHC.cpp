 ...
 */
/*
 * Diagnostic O2Micro build:
 * keep the legacy SDMA implementation compiled for later comparison, but
 * force the normal transfer path through PIO and do not set SDHCI_TRNS_DMA.
 */
#define USE_SDMA 0

#define SDHCI_RESET_TIMEOUT_LOOPS   10000U
#define SDHCI_COMMAND_TIMEOUT_LOOPS 100000U
#define SDHCI_CLOCK_TIMEOUT_LOOPS   100000U
#define SDHCI_STATUS_TIMEOUT_LOOPS  500000U
...
bool VoodooSDHC::start ( IOService * provider )
{
	...
	IOPCIDevice *pciProvider = OSDynamicCast(IOPCIDevice, provider);
	if (pciProvider) {
		UInt16 vendorID = pciProvider->configRead16(kIOPCIConfigVendorID);
		UInt16 deviceID = pciProvider->configRead16(kIOPCIConfigDeviceID);
		IOLog("VoodooSDHCI: PCI controller %04X:%04X\n", vendorID, deviceID);
		if (vendorID == 0x1217 && deviceID == 0x7120) {
			IOLog("VoodooSDHCI: O2Micro 1217:7120 diagnostic path: PIO/read-only\n");
		}
	}
...
bool VoodooSDHC::cardInit(UInt8 slot)
{
	...
	if (!SDCommand(slot, SD_GO_IDLE_STATE, SDCR0, 0))
			return false;
	IODelay(30000);
	if (!SDCommand(slot, SD_SEND_IF_COND, SDCR8, 0x000001AA))
			return false;
	for (int i = 0; i < 100; i++) {
		IODelay(10000);
	...
	if(this->PCIRegP[slot]->PresentState & ComInhibitCMD) {
		...
		if (!SDCommand(slot, SD_GO_IDLE_STATE, SDCR0, 0))
			return false;
		bool opCondComplete = false;
		for (int i = 0; i < 80; i++) {
			if (!SDCommand(slot, SD_APP_CMD, SDCR55, 0))
			return false;
			if (!SDCommand(slot, SD_APP_OP_COND, SDACR41, 0x00FF8000))
			return false;
			IODelay(1000);
...
#endif //me
	if (!SDCommand(0, SD_APP_CMD, SDCR55, this->RCA << 16))
			return false;
	if (!SDCommand(slot, SD_APP_SET_BUS_WIDTH, SDCR6, 2))
			return false;
	IODelay(30000);
#ifdef	__DEBUG__
	IOLog("VoodooSDHCI: PCIRegP response order (3,2,1,0) :: 0x%08X 0x%08X 0x%08X 0x%08X\n", 
...
#endif //me
	if (!SDCommand(slot, SD_SWITCH, SDCR6, 0x01fffff1))
			return false;
	IODelay(10000);
	if (!calcClock(slot, 50000000))
			return false;
 ...
 */
bool VoodooSDHC::Reset(UInt8 slot, UInt8 type)
{
	switch(type) {
		case CMD_RESET:
	...
	for (UInt32 i = 0; i < SDHCI_RESET_TIMEOUT_LOOPS; i++) {
		if (!this->PCIRegP[slot]->SoftwareReset)
			return true;
		IODelay(10);
	}
	IOLog("VoodooSDHCI: reset timeout, type=0x%02X\n", type);
	dumpRegs(slot);
	return false;
}
...
bool VoodooSDHC::SDCommand(UInt8 slot, UInt8 command, UInt16 response,
								UInt32 arg) {
	if (command != 0) {
		for (UInt32 i = 0; i < SDHCI_COMMAND_TIMEOUT_LOOPS; i++) {
			if (!(this->PCIRegP[slot]->PresentState & ComInhibitCMD))
				break;
			IODelay(10);
		}
		if (this->PCIRegP[slot]->PresentState & ComInhibitCMD) {
			IOLog("VoodooSDHCI: CMD%u timeout waiting for ComInhibitCMD to clear\n", command);
			dumpRegs(slot);
			if (!Reset(slot, CMD_RESET))
				IOLog("VoodooSDHCI: CMD%u timeout reset also failed\n", command);
			return false;
		}
	}

#ifdef __DEBUG__
	IOLog("VoodooSDHCI: CMD%u arg=0x%08X pre-state=0x%08X nis=0x%04X eis=0x%04X\n",
		command, arg, this->PCIRegP[slot]->PresentState,
		this->PCIRegP[slot]->NormalIntStatus, this->PCIRegP[slot]->ErrorIntStatus);
#endif
...
#ifdef __DEBUG__
	IOLog("VoodooSDHCI: CMD%u issued cmdreg=0x%04X state=0x%08X nis=0x%04X eis=0x%04X rsp0=0x%08X\n",
		command, this->PCIRegP[slot]->Command, this->PCIRegP[slot]->PresentState,
		this->PCIRegP[slot]->NormalIntStatus, this->PCIRegP[slot]->ErrorIntStatus,
		this->PCIRegP[slot]->Response[0]);
#endif
	return true;
...
bool VoodooSDHC::calcClock(UInt8 slot, UInt32 clockspeed) {
	...
	for (UInt32 i = 0; i < SDHCI_CLOCK_TIMEOUT_LOOPS; i++) {
		if (this->PCIRegP[slot]->ClockControl & BIT1)
			break;
		IODelay(10);
	}
	if (!(this->PCIRegP[slot]->ClockControl & BIT1)) {
		IOLog("VoodooSDHCI: timeout waiting for internal clock stable\n");
		dumpRegs(slot);
		return false;
	}
...
{
	for (UInt32 cnt = 0; cnt < SDHCI_STATUS_TIMEOUT_LOOPS; cnt++) {
		UInt32 nis = PCIRegP[0]->NormalIntStatus;
		if (nis & ErrorInterrupt) {
			IOLog("VoodooSDHCI: interrupt error while waiting for mask 0x%08X: nis=0x%04X eis=0x%04X\n",
				maskBits, nis, PCIRegP[0]->ErrorIntStatus);
			dumpRegs(0);
			return false;
		}
		if (nis & maskBits) {
	...
	IOLog("VoodooSDHCI: interrupt timeout waiting for mask 0x%08X: nis=0x%04X eis=0x%04X\n",
		maskBits, PCIRegP[0]->NormalIntStatus, PCIRegP[0]->ErrorIntStatus);
	dumpRegs(0);
	return false;
}
